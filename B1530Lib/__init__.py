...
from B1530Lib.extlibs.stdout_redirect import stdout_redirector
...
class Waveform:
    ...
    def append_wait_end(self, new_total_duration=None, wait_time=None, voltage=None):
        ...
        if new_total_duration is None == wait_time is None:
            raise ValueError(
                "One and only one of new_total_duration or wait_time expected"
            )
    ...
    def prepend_wait_begin(self, new_total_duration=None, wait_time=None, voltage=None):
        """
    ...
    def measure(
        ...
        **kwargs,
        ...
        measurement_times = np.arange(
            meas.start_delay, meas.get_total_duration(), meas.sample_interval
        )
        sampled_pattern = np.interp(
            measurement_times,
            np.cumsum(self.get_time_pattern()),
            self.get_voltage_pattern(),
        )
        change_samples = np.nonzero(
            np.abs(sampled_pattern[:-1] - sampled_pattern[1:]) > 1e-8
        )[0]
        ...
        if ignore_settling:
            for i in range(len(change_samples)):
                ...
                if (
                    i + 1 == len(change_samples)
                    or abs(change_samples[i] - change_samples[i + 1]) > 1
                ):
                    settle_start = change_samples[i] + 1
                    settle_end = min(
                        settle_start + settling_sample_count, len(measurement_times) - 1
                    )

                    to_ignore = np.append(
                        to_ignore, range(settle_start, settle_end)
                    ).astype(int)
        ...
        while (
            ignored_start_index < len(to_ignore)
            and to_ignore[ignored_start_index] == ignored_start_index
        ):
        ...
        ignored_end_index = 1
        while (
            ignored_end_index < len(to_ignore)
            and to_ignore[-ignored_end_index]
            == len(measurement_times) - ignored_end_index
        ):
            ignored_end_index += 1
        ...
        if ignored_start_index > 0 or ignored_end_index > 1:
            ...
            meas.duration -= (
                ignored_start_index + ignored_end_index - 1
            ) * meas.sample_interval

            to_ignore = (
                to_ignore[ignored_start_index:-ignored_end_index] - ignored_start_index
            )
    ...
    def get_max_abs_voltage(self):
        return ft.reduce(lambda a, b: a if abs(a[1]) > abs(b[1]) else b, self.pattern)[
            1
        ]
    ...
    def get_cleansed(self):
        ...
        """
        pattern = list(filter(lambda p: p[0] != 0, self.pattern))
...
class Pulse(Waveform):
    ...
    def __init__(
        ...
        if interval is None:
            if wait_begin is None or wait_end is None:
                raise ValueError(
                    "Either interval or wait_begin/end expected, none got provided"
                )
        else:
            if wait_begin is not None or wait_end is not None:
                raise ValueError(
                    "Either interval or wait_begin/end expected, both got provided"
                )
    ...
    def reduced_length(self, length, **kwargs):
        ...
        if diff < 0:
            raise ValueError(
                "Cannot increase length, directly used length attribute instead"
            )
    ...
    def __gen(self):
        ...
        for i in range(self._step_count):
            self.pattern.extend(
                [
                    [
                        self._jump_time if i != 0 else 0,
                        self._init_voltage + i * step_voltage,
                    ],
                    [self._step_length, self._init_voltage + i * step_voltage],
                ]
            )
        ...
        if self._end_reset:
            self.pattern.extend(
                [
                    [self._jump_time, self._init_voltage],
                    [self._step_length / 2, self._init_voltage],
                ]
            )
...
class Measurement:
    ...
    def __init__(
        self, mode, range, average_time, sample_interval, start_delay, duration
    ):
        if mode != "current" and mode != "voltage":
            raise ValueError(
                f"Unknown specified mode: '{mode}', 'voltage' or 'current' expected"
            )
    ...
    def duration(self, new_duration):
        self._duration = (
            int(new_duration / self.sample_interval) * self.sample_interval
        )  # To have a multiple of the sample interval
    ...
    def get_id_at(self, time):
        ...
        if relative_time > self.duration:
            ...
            raise ValueError(
                "Time which is not greater than start_delay does not have a sample id"
            )
...
class B1530:
    ...
    * Provides high-level helper functions to configure, execute, retrieve and manipulate measurements
    ...
    def __init__(self, addr=DEFAULT_ADDR):
        ...
        b1530_methods = [
            func
            for func in dir(B1530Driver)
            if callable(getattr(B1530Driver, func)) and not func.startswith("__")
        ]
        for method in b1530_methods:
            ...
            def wrapper_gen(func):
                def call_wrapper(*args, **kwargs):
                    err_buf = io.BytesIO()
                    has_err = False
                    with stdout_redirector(err_buf):
    ...
    def configure(self, pattern_count=1):
        """
        Applies the WGFMUs configuration set with wgfmu.wave/meas.

        Parameters:
                pattern_count: int : how many times to repeat the pattern
        """
        self.d_clear()
        ...
        for i, channel in self.chan.items():
            if channel.wave is None:
                if channel.meas is None:
                    # We disconnect the channel if it was previously connected
                    if self.active_chan[i]:
                        self.d_disconnect(channel.id)

                    self.active_chan[i] = False
                    continue
                else:  # If there is a meas, we overwrite the pattern accordingly so
                    channel.wave = Waveform(
                        [[0, 0], [channel.meas.get_total_duration(), 0]]
                    )
            ...
            if len(filtered_wf.pattern) > 1:
                ...
                self.d_addVector(
                    self.pattern_name[i],
                    filtered_wf.pattern[0][0],
                    filtered_wf.pattern[0][1],
                )
            ...
            operation_mode = None
            if meas is not None:
                operation_mode = B1530Driver._operationMode[
                    "fastiv"
                ]  # If we have a measurement, we can only use fastiv mode
                ...
                operation_mode = (
                    B1530Driver._operationMode["pg"]
                    if not wf.force_fastiv
                    else B1530Driver._operationMode["fastiv"]
                )

            # Link config to the chan
            self.d_addSequence(channel.id, self.pattern_name[i], self._pattern_count)
            ...
            if meas is not None:
                ...
                if mode == "voltage":
                    self.d_setForceVoltageRange(
                        channel.id, B1530Driver._forceVoltageRange["auto"]
                    )
                    self.d_setMeasureVoltageRange(
                        channel.id, B1530Driver._measureVoltageRange[channel.meas.range]
                    )
                elif mode == "current":
                    self.d_setMeasureCurrentRange(
                        channel.id, B1530Driver._measureCurrentRange[channel.meas.range]
                    )
                else:
    ...
    def exec(self, concat_repetitions=False, wait_until_completed=True):
        ...
        if wait_until_completed:
            ...
            for channel in meas_chan.values():
                meas_point_count = channel.meas.get_count()
                data = [None] * self._pattern_count  # [pd.DataFrame]

                time, meas = self.d_getMeasureValues(
                    channel.id, 0, meas_point_count * self._pattern_count
                )

                for i in range(self._pattern_count):
                    start_id = meas_point_count * i
                    end_id = start_id + meas_point_count
                    ...
                    df["time" + channel.name] = list(
                        map(
                            lambda t: t - channel.meas.average_time / 2,
                            time[start_id:end_id],
                        )
                    )
                    df[channel.name] = meas[start_id:end_id]

                    data[i] = df.drop(channel.meas.ignore_sample).reset_index(drop=True)
            ...
            for arg_idx, arg in enumerate(args):
                ...
                if chan is None:
                    ...
                    if chan.meas is None:
                        raise ValueError(
                            "Trying to retrieve measures of a non-measure chan '"
                            + chan.name
                            + "'"
                        )
                ...
                if pattern_count == 1:
                    ...
                    renamed = chan.meas.result[res_idx].rename(
                        {chan_time_name: "time"}, axis=1
                    )
                ...
                if result[res_idx] is None:
                    ...
                    result[res_idx] = pd.merge_asof(
                        result[res_idx],
                        renamed,
                        on="time",
                        direction="nearest",
                        tolerance=1e-8,
                    )  # 1e-8s == 10ns is the shortest time scale the B1530 can handle
